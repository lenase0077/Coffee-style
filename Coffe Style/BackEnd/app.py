...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://zole:sinpezca2@zole.mysql.pythonanywhere-services.com/zole$comentarios"
...
class Comentario(db.Model):  # Producto hereda de db.Model comentarios era antes productos
    """
    ...
    def __init__(self, nombre, imagen, mensaje):
        """
        Constructor de la clase Producto.

        Args:
            nombre (str): Nombre del producto.
            mensaje (int): mensaje del producto.
...
class ComentariosSchema(ma.Schema):
    ...
    """
    class Meta:
        fields = ("id", "nombre", "imagen", "mensaje" )

comentarios_schema = ComentariosSchema()  # Objeto para serializar/deserializar un producto
comentarios_schema = ComentariosSchema(many=True)  # Objeto para serializar/deserializar múltiples productos
...
def get_Comentarios():
    ...
    """
    all_coments = Comentarios.query.all()  # Obtiene todos los registros de la tabla de productos
    result = comentarios_schema.dump(all_coments)  # Serializa los registros en formato JSON
...
def delete_comentario(id):
    ...
    """
    all_coments = Comentario.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    db.session.delete(comentario)  # Elimina el producto de la sesión de la base de datos
...
def create_comentario():
    ...
    mensaje = request.json["mensaje"]  # Obtiene el mensaje del producto del JSON proporcionado
   
    new_comentario = Comentario(nombre, imagen, mensaje)  # Crea un nuevo objeto Producto con los datos proporcionados
    db.session.add(new_comentario)  # Agrega el nuevo producto a la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
...
def update_comentario(id):
    ...
    Retorna un JSON con el producto actualizado.
    """
    comentario = Comentario.query.get(id)  # Obtiene el producto existente con el ID especificado

    # Actualiza los atributos del producto con los datos proporcionados en el JSON
    comentario.nombre = request.json["nombre"]
    comentario.imagen = request.json["imagen"]
    comentario.mensaje = request.json["mensaje"]
    
    

    db.session.commit()  # Guarda los cambios en la base de datos
    return comentarios_schema.jsonify(comentario)  # Retorna el JSON del producto actualizado
